...
$\mathcal{O}(N\log N)$ one. On top of this representation, we introduce an adaptive environment map, split over near-convex
parts, and a compact FastKAN decoder to model sharp view-dependent appearance; the resulting
quality gains come from these two modules, not from the representation itself, which only removes
the sort-order failure above. Given the mesh, appearance now composes directly with the traditional
pipeline: we reach $36.99$ dB PSNR on Shiny-Blender and render at $76.8$ FPS on a consumer RTX 4090
with a $1.74$ GB footprint. Because the anchoring is linear in the vertices, appearance
