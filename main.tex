...
used: adding primitives adds no angular bandwidth. We resolve both with Automatic Neural Baking. An
\emph{on-manifold kernel field}, 2D Gaussian disks anchored to mesh faces and blended by an
order-free partition of unity, removes the ordering obstruction; factoring view dependence out of
the elements into a shared environment map and a FastKAN decoder removes the locality bound. The
result composes directly with the traditional rendering pipeline, baking splat-quality, real-time
appearance onto a mesh.
\end{abstract}
