...
at the same time devotes explicit model capacity to sharp specularity.
In addition, methods above presupposes a global UV atlas,
...
\citep{neumesh} is relevant in a sense that it binds learnable codes on the vertices, but never allow the code representation to float inside the 2D manifold. 
While original EWA surface splatting \cite{EWA} is not differentiable.
