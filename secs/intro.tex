...
$(\varepsilon_i-\varepsilon_j)(\mathbf{n}\cdot\mathbf{v})$, where $\mathbf{n}$ is the face normal and $\mathbf{v}$ the viewing direction. 
With small offsets, pairs still flip at views reaching high elevations, and flips are densest where
the surface faces the camera; larger offsets push the flips toward grazing angles but never remove
them, so gradients from different views still disagree about which primitive owns a fragment.
...
space at every fragment, and adding primitives adds no angular bandwidth to escape it (Theorem A1,
...
invariant to the order of its terms, the coplanar degeneracy cannot arise. This is a resolution, not
the resolution: a fixed-index order also satisfies Theorem B, and whether it can be paired with the
same shared decoder to match this representation is a question we leave open (App.~\ref{app:theory}).
