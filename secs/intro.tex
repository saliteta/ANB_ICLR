...
\caption{%
\textbf{Automatic Neural Baking bakes highly reflective, view-dependent appearance directly onto a mesh, rendering it in real time ($\geq$ 60 FPS) with fidelity that surpasses state-of-the-art flexible-mesh methods.}
...
\emph{Pipeline}: from posed reference images ~\badgeimg{} and the corresponding triangle mesh~\badgemesh, we optimize three interchangeable, rasterizer-native modules - an on-manifold 2D Gaussian mixture storing diffuse albedo and reflectivity~\badgegmm, an adaptive, hierarchical environment map capturing self-reflection ~\badgeenv; and a lightweight KAN that evaluates the reflection equation for far-field specular~\badgekan - all evaluated per-fragment in a standard fragment shader
...
a field whose elements are individually addressable and differentiable and whose domain is the
surface itself, rather than a distribution in ambient $\mathbb{R}^3$.
...
\item \textbf{Serves as a texture.} By construction, our field serves as a texture that the
traditional pipeline reads directly. We realize it in nvdiffrast and render both datasets at over
$60$ FPS, while reaching the novel-view-synthesis quality of splat representations.
% $\partial\mathbf{x}/\partial \mathbf{V} = B$,%
\item \textbf{Differentiable in the mesh.} The anchoring is linear in the vertices, so appearance
gradients have an exact path to the mesh,  a path no primitive
floating in $\mathbb{R}^3$ possesses. We show this path is degenerate along the surface normal,
derive how the missing direction is recovered without leaving the surface, and verify the
derivation empirically: appearance gradients alone reduce the normal error of a coarse extracted
mesh on all six Shiny-Blender scenes (Sec.~\ref{sec:refinement}).
\end{itemize}