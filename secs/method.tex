...
We analyze the differentiable nature of our representation. Let the mesh have vertices
$\mathbf{V}$ with vertex normals $\mathbf{n}_v$, and faces $f$ with
face normals $\mathbf{n}_f$. Stacking Eq.~\eqref{eq:center} over all $N$ disks, the anchoring is
linear in the vertices, $\mathbf{X} = \mathbf{B}\mathbf{V}$, where the sparse matrix $\mathbf{B}$
holds the barycentric weights, so appearance gradients have an exact path to the mesh. However, we note that the path is degenerate in one direction: the blend weights of Eq.~\eqref{eq:pou} see only in-plane
