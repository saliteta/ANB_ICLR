...
The mesh is fixed; only the appearance on it is optimized.

We bake appearance with three modules and composite their outputs at each fragment (Figure~\ref{fig:method}).
...
Its center is \emph{reconstructed} from the live vertices $\mathbf{p}_0(f_i), \mathbf{p}_1(f_i), \mathbf{p}_2(f_i)$ of that triangle,
%
\begin{equation}
...
every \emph{edge} rather than a fixed nonlinearity on every node~\citep{kan}, realized with Gaussian
radial basis functions on a small fixed grid~\citep{fastkan}; on our task it outperforms a
parameter-matched MLP by $2.44$ dB (Sec.~\ref{sec:ablation}). Where inference cost matters more than
