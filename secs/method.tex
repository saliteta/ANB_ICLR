...
We analyze the differentiable nature of our representation. A mesh $M$ is charactereized by its vertices $\mathbf{V} \in \mathbb{R}^{M \times 3}$ and faces $\mathbf{F} \in \mathbb{R}^{M \times 3}$. Each of these have a normal and $n_v$ and $n_f$. The anchoring of Eq.~\eqref{eq:center} is linear in the vertices $\mathbf{V}$ (i.e.\ $\mathbf{X} = \mathbf{B}\mathbf{V}$
with $\mathbf{B}$ the barycentric weights), so appearance gradients have an exact path to the mesh. However, we note that the path is degenerate in one direction: the blend weights of Eq.~\eqref{eq:pou} see only in-plane
offsets, so $\langle \partial w_i / \partial \mathbf{x}_i,\ \mathbf{n}_i \rangle \equiv 0$ and a
...
Due to differentiability of our representation, we can potentially refine the underlying mesh that is given.More details of this aspect of our representation our in Appendix \ref{app:refinement}.
