...
\label{sec:ordering}
Sec.~\ref{sec:intro} argues that binding floating splats to a mesh face makes them coplanar, so
alpha compositing has no well-defined order, and that the standard remedy, offsetting each
primitive along the face normal, does not fix this: it only trades an undefined order for an
arbitrary one. We test this empirically. Starting from the 2DGS$^{*}$ baseline (2D Gaussians
initialized evenly on the mesh surface, with every gradient projected onto the manifold so a disk
can slide along its face but never leave it), we additionally displace each disk by a small, fixed
offset $\varepsilon$ along the face normal at initialization; because normal motion is disallowed
during training, this offset persists and gives every disk a fixed depth order for compositing. We
sweep $\varepsilon\in\{10^{-5},10^{-4},10^{-3}\}$ across all six Shiny-Blender scenes.
Table~\ref{tab:ordering} shows PSNR is flat across three orders of magnitude of
$\varepsilon$ and no better than the untied $\varepsilon{=}0$ baseline: fixing an arbitrary order
does not recover the quality a geometrically meaningful one would, confirming that the obstruction
is order-dependence itself, not depth.
\input{tabs/ordering_degeneracy}

\vspace{-2mm}
\subsection{Appearance Quality}
\label{sec:quality}
% Given the mesh, our renders reproduce sharp, high-frequency environment reflections on metallic
% surfaces (Figure~\ref{fig:qualitative}), reaching $38.04$~dB PSNR on Shiny-Blender and $33.74$~dB
% on NeRF-Synthetic (Table~\ref{tab:main}). The fixed-mesh rows share our input mesh and are the only
% ones we compare against directly; the unconstrained and flexible-mesh rows reconstruct their own
% geometry and are shown as context. Across scenes, our margin over the best context method is
% smallest on \emph{teapot} ($+0.45$~dB) and \emph{coffee} ($+0.72$~dB)
% (Table~\ref{tab:shiny_perscene}).

Given the mesh, our appearance field reaches $38.04$~dB PSNR on Shiny-Blender and $33.74$~dB on
NeRF-Synthetic (Table~\ref{tab:main}) -- not just the best fixed-mesh result, but the best result
in the table overall: on Shiny-Blender we surpass RefGS and 3DGS-DR, the best flexible-mesh and
unconstrained methods, by $3.24$ and $4.10$~dB, despite their freedom to reconstruct their own
geometry from scratch. The margin over the best context method (Table~\ref{tab:shiny_perscene})
tracks how reflective a scene is, from $+8.91$~dB on the near-mirror \emph{ball} down to
$+0.45$~dB on \emph{teapot}, whose highlights are smallest. Qualitatively
(Figure~\ref{fig:qualitative}), only our renders reproduce the sharp, high-frequency reflections
that 2DGS$^{*}$ and DNR blur and RefGS recovers less crisply.
...
4090 (WSL2, Ubuntu 22.04), with a mean CUDA memory footprint of $1.74$ GB peak at $2.75$ GB, all under \texttt{torch.compile}.

% The full asset ($10^6$ disks, five environment maps, shader) occupies $48.3$ MB on disk. At
% $800{\times}800$ we render the test views at $91.7$ FPS on NeRF-Synthetic and $68.6$ FPS on
% Shiny-Blender on an NVIDIA H100 (CUDA 12.8), and at $76.8$ FPS on Shiny-Blender on a consumer RTX
% 4090 (WSL2, Ubuntu 22.04), with a mean CUDA memory footprint of $1.74$ GB peak at $2.75$ GB. All measurements use \texttt{torch.compile}; the H100 figures are from a shared datacenter node.
