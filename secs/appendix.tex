...
vertices $\mathbf{V}$ are then refined against
\begin{equation}
\mathcal{L}(\mathbf{V}) = \mathcal{L}_{\text{app}} + \lambda_{\text{nrm}}\,\mathcal{L}_{\text{nrm}},
\qquad
\mathcal{L}_{\text{nrm}} = \frac{\sum_f A_f\,\big(1 - \mathbf{n}_f(\mathbf{V})^{\top}\bar{\mathbf{n}}_f\big)}{\sum_f A_f},
\label{eq:refine_loss}
\end{equation}
where $\mathcal{L}_{\text{app}}$ is the photometric loss of Eq.~\eqref{eq:loss} and
$\mathcal{L}_{\text{nrm}}$ is a normal-consistency prior between each face's geometric normal
$\mathbf{n}_f(\mathbf{V})$ and $\bar{\mathbf{n}}_f$, the normalized mean of the input mesh's vertex
normals $\mathbf{n}_v$ at that face (held fixed); $A_f$ is the face area (treated as a constant).
We empirically set $\lambda_{\text{nrm}} = 0.5$, relative to the norm of the photometric gradient.
We alternate 300 cycles of a geometry step, which moves each vertex along its normal by a smoothed,
capped update whose size is set relative to the initial disk scale $s_0$ from which the per-disk
scales $s_{i,j}$ are initialized, and an appearance step on the disks with the mesh fixed.
Table~\ref{tab:refine_decomp} shows that this consistency term is necessary: the photometric
gradient alone degrades face normals ($7.20^\circ \rightarrow 18.58^\circ$), while the prior alone
does not use the images and moves the surface away from the ground truth; only the two together
improve on the input mesh.

\paragraph{Metrics.} Geometry is scored against the ground-truth Blender models. Normals are
scored by mean angular error against the ground-truth normal maps of the 200 test views. To assess
